...
public class IdentityService : IIdentityService
{
    ...
    public async Task<IResult> CreateStaff(CreateStaffDTO createStaffDto)
    {
        var user = createStaffDto.Adapt<Staff>();
        user.UserName = Guid.NewGuid().ToString();

        var result = await _applicationUserManager.CreateAsync(user, createStaffDto.Password);
        if(!result.Succeeded) return Result.Failure();

        var createdStaff = await _applicationUserManager.FindByEmailAsync(user.Email!);
        if(createdStaff is null) return Result.Failure();

        var roleAssigned = await _applicationUserManager.AddToRoleAsync(createdStaff, createStaffDto.Role.ToString());
        if(!roleAssigned.Succeeded) return Result.Failure();
